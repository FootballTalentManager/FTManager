...
<html lang="pt">
    <head>
        ...
        <title>Football Talent Manager - As minhas Equipas</title>
    ...
    <body class="sb-nav-fixed">
                    ...
                    <div class="container-fluid px-4">
                        <div class="row" style="border-bottom: 2px solid black; padding-bottom: 1rem;">
                            <div class="col-md-auto">
                                <h2 class="mt-4"><span class="iconify-inline" data-icon="bi:shield-shaded" data-width="30" data-height="30"></span>  As minhas Equipas</h2>
                            </div>
                        </div>
                            ...
                            <div class="card mb-4" style="padding-right: 0; padding-left: 0;">
                                ...
                                <div class="card-body">
                                    <table id="table" class="table table-hover" style="width:100% !important;">
                                        ...
                                        <tbody>
                                        <?php
                                        $queryGetEquipaTecnica = "SELECT * FROM `$dataBaseName`.`equipatecnica` WHERE `IDTreinador` = '$IDUser'";
                                        $queryResult = mysqli_query($GLOBALS['ligacao'], $queryGetEquipaTecnica);

                                        if ($queryResult->num_rows > 0){
                                            while($row = mysqli_fetch_row($queryResult)){
                                                $queryGetEquipas = "SELECT * FROM `$dataBaseName`.`equipa` WHERE `IDEquipa` = '$row[0]'";
                                                $queryResultGetEquipas = mysqli_query($GLOBALS['ligacao'], $queryGetEquipas);

                                                if ($queryResultGetEquipas->num_rows > 0) {
                                                    while ($row = mysqli_fetch_row($queryResultGetEquipas)) {
                                                        ?>
                                                        <tr style="cursor: pointer;" id="<?php echo $row[0] ?>">
                                    ...
                                    <form method="post" action="aMinhaEquipa.php" id="tableForm">
                                        <input type="hidden" id="input-epoca" name="input-epoca">
                                        <input type="hidden" id="input-escalao" name="input-escalao">
                                        <input type="hidden" id="input-nivelCompeticao" name="input-nivelCompeticao">
                                        <input type="hidden" id="IDEquipa" name="IDEquipa">
                                    </form>
                                </div>
                            </div>
                        </div>
        ...
        <script src="js/dataTables/dataTable-asMinhasEquipas.js"></script>
        <script>
            function getEpocas (){
