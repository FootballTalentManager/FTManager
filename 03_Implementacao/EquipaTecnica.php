...
<head>
    ...
    <title>Football Talent Manager - Equipa Tecnica</title>
            ...
            <div class="container-fluid px-4">
                <div class="row" style="border-bottom: 2px solid black; padding-bottom: 1rem;">
                    <div class="col-md-auto">
                        <h2 class="mt-4"><i class="fas fa-book-reader"></i>  <?php echo (strtoupper($escalao) . " " . strtoupper($nivelCompeticao) . " (" . $NomeEpoca . ")"); ?></h2>
                    </div>
                </div>
                <div class="row" style="margin-top: 2rem !important">
                    <div class="card mb-4" style="padding-right: 0; padding-left: 0">
                        <div class="card-header">
                            <h6><span class="iconify-inline" data-icon="codicon:table" data-width="20" data-height="20"></span>  Equipa Técnica</h6>
                        </div>
                        <div class="card-body">
                            <table id="table" class="table table-striped" style="width:100% !important;">
                                ...
                                <tbody>
                                <?php
                                    $queryIDEquipa = "SELECT `IDEquipa` FROM `$dataBaseName`.`equipa` WHERE `NomeEpoca` = '$NomeEpoca' AND `Escalao` = '$escalao' AND `NivelCompeticao` = '$nivelCompeticao'";
                                    $queryResult = mysqli_query($GLOBALS['ligacao'], $queryIDEquipa);
                                    ...
                                    if ($queryResult->num_rows > 0){
                                        ...
                                        $queryIDEquipa = "SELECT * FROM `$dataBaseName`.`equipatecnica` WHERE `IDEquipa` = '$IDEquipa'";
                                        $queryResult = mysqli_query($GLOBALS['ligacao'], $queryIDEquipa);

                                        if ($queryResult->num_rows > 0){
                                            while($row = mysqli_fetch_row($queryResult)){
                                                $queryNomeUtilizador = "SELECT `Nome` FROM `$dataBaseName`.`utilizador` WHERE `IDUtilizador` = '$row[1]'";
                                                $queryResultGetNomeUtilizador = mysqli_query($GLOBALS['ligacao'], $queryNomeUtilizador);
                                                while ($nomeUser = mysqli_fetch_row($queryResultGetNomeUtilizador)) {
                                                    ?>
                                                    <tr>
                                                        <td><?php echo $nomeUser[0]?></td>
                                                        <td><?php echo $row[2]?></td>
                                                        <td><?php echo $row[3]?></td>
                                                        <?php if ($row[1] === $IDUser){ ?>
                                                            <td> <input type="button" class="smallSkewBtn brick" id="btn-finalizar" value="finalizar"> </td>
                                                        <?php } else { ?>
                                                            <td></td>
                                                        <?php } ?>
                                                    </tr>
                                            ...
                                            $queryVerifyTreinador = "SELECT EXISTS(SELECT `IDTreinador` FROM `$dataBaseName`.`equipatecnica` WHERE `IDEquipa` = '$IDEquipa' AND `IDTreinador` = '$IDUser')";
                                            $queryResult = mysqli_query($GLOBALS['ligacao'], $queryVerifyTreinador);
                                            if ($queryResult->num_rows > 0){
                                                while($row = mysqli_fetch_row($queryResult)){
                                                    $istrainadorThisTeam = $row[0];
                                                }
                                            }
                                        }
                                    }
                            ...
                            <form method="post" name="formEquipaTecnica" action="submitTreinadorEquipaTecnica.php">
                                <input type="hidden" value="<?php echo $Inputs;?>" id="hiddenField" name="hiddenField"/>
                            </form>
                            <form method="post" name="formfinalizarEquipaTecnica" action="formfinalizarEquipaTecnica.php">
                                <input type="hidden" value="<?php echo $Inputs;?>" id="hiddenField" name="hiddenField"/>
                            </form>
                        </div>
                    </div>
...
<script src="js/dataTables/dataTable-EquipaTecnica.js"></script>
<script>
    ...
    $(document).ready(function() {
        ...
        let isTreinador = <?php echo json_encode($istrainadorThisTeam); ?>;
        isTreinador = ((isTreinador === "0" || isTreinador === false ) ? false : true);
        console.log("Is treinador " + isTreinador);
        if (isTreinador){
            document.getElementById("addTreinador").disabled = true;
        } else {
            $('#addTreinador').click( function () {
                formEquipaTecnica.submit();
            } );
        }

        if (document.getElementById('btn-finalizar') !== null){
            $('#btn-finalizar').click( function () {
                formfinalizarEquipaTecnica.submit();
            } );
        }
    } );
</script>
