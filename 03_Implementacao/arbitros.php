...
<head>
    ...
    <title>Football Talent Manager - Arbitros</title>
            ...
            <div class="container-fluid px-4">
                <div class="row" style="border-bottom: 2px solid black; padding-bottom: 1rem;">
                    <div class="col-md-auto">
                        <h2 class="mt-4"><span class="iconify-inline" data-icon="bpmn:user" data-width="40" data-height="40"></span>  Árbitros</h2>
                    </div>
                </div>
                ...
                <div class="row" style="margin-top: 2rem !important">
                    <div class="card mb-4" style="padding-right: 0; padding-left: 0;">
                        <div class="card-header">
                            <h6><span class="iconify-inline" data-icon="codicon:table" data-width="20" data-height="20"></span>  Árbitros</h6>
                        </div>
                        <div class="card-body">
                            <form method="post" action="formArbitros.php" id="tableForm" enctype="multipart/form-data">
                                <table id="table" class="table table-hover" style="width:100% !important;">
                                    ...
                                    <tr style="color: white; background-color: #34495E">
                                        ...
                                        <th>Cedula</th>
                                        <th>Associação</th>
                                        <th>Classificação Média Por Jogo</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php
                                        $queryGetEquipaTecnica = "SELECT * FROM `$dataBaseName`.`arbitro`";
                                        $queryResult = mysqli_query($GLOBALS['ligacao'], $queryGetEquipaTecnica);
                                            ...
                                            while($row = mysqli_fetch_row($queryResult)){
                                                ...
                                                if ($queryResultJogo->num_rows > 0) {
                                                    while ($jogo = mysqli_fetch_row($queryResultJogo)) {
                                                        ...
                                                        $result = $jogo->xpath("//preInformacao/equipaArbitragem[@idArbitroPrincipal='" . $idArbitro . "' or @idArbitroAss1='" . $idArbitro . "' or @idArbitroAss2='" . $idArbitro . "' or @idQuartoArbitro='" . $idArbitro . "']");

                                                        if (count($result) > 0) {
                                                            $nTotalDeJogos++;

                                                            $result = $jogo->xpath("//posInformacao/registosFinaisArbitros/registoFinalArbitro[@idArbitro='" . $idArbitro . "']");
                                                            $classificacaoArbitro += (int)$result[0]['classificacao'];
                                                        }
                                                    }
                                                }
                                                ?>
                                                <tr class="clickable-row" id="<?php echo $row[0] ?>">
                                                    <td><img src="showImage.php?imageURL=<?php echo $urlFoto; ?>" style="width: 50px; height: auto" alt=""></td>
                                                    <td><?php echo $Nome?></td>
                                                    <td><?php echo $Cedula?></td>
                                                    <td><?php echo $Associacao?></td>
                                                    <td><?php if($classificacaoArbitro === 0 || $nTotalDeJogos === 0){ echo 0;} else{ echo round(($classificacaoArbitro/$nTotalDeJogos), 2);}?></td>
                                                </tr>
                                                <?php
                                            }
                                        }
...
<script src="js/dataTables/dataTable-arbitros.js"></script>

<script>
    function isTreinador(){
        let isCoach = [<?php echo json_encode($isTreinador); ?>]
        return isCoach;
    }
