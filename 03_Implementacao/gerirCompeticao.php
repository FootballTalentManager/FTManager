...
$queryGetEpocas = "SELECT `NomeEpoca` FROM `$dataBaseName`.`epoca`";
$queryResult = mysqli_query($GLOBALS['ligacao'], $queryGetEpocas);

if ($queryResult->num_rows > 0){
    while($row = mysqli_fetch_row($queryResult)){
        array_push($epocas, $row[0]);
    }
}
...
?>
    ...
    <head>
        ...
        <title>Football Talent Manager - Gerir Competições</title>
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <div class="row">
                        <div class="row" id="cardSuccess" hidden>
                            <div class="card bg-success text-white">
                                <div class="card-body" style="padding-top: 30px;"> <h6><i class="far fa-thumbs-up"></i> - Equipa removida com sucesso.</h6></div>
                            </div>
                        </div>
                        <div class="row" id="cardDanger" hidden>
                            <div class="card bg-danger text-white mb-4">
                                <div class="card-body" style="padding-top: 30px;"><i class="fas fa-exclamation-triangle"></i> - Lamentamos mas algo aconteceu de errado, por favor tente novamente!</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row" style="border-bottom: 2px solid black; padding-bottom: 1rem;">
                        <div class="col-md-auto">
                            <h2 class="mt-4"><span class="iconify-inline" data-icon="whh:managedhosting" data-width="30" data-height="30"></span></span>  Gerir Competições</h2>
                        </div>
                    </div>
                    ...
                    <div class="row" style="margin-top: 2rem !important">
                        <div class="card mb-4" style="padding-right: 0; padding-left: 0;">
                            <div class="card-header">
                                <h6><span class="iconify-inline" data-icon="whh:notestasks" data-width="20" data-height="20"></span>  Competições</h6>
                            </div>
                            <div class="card-body">
                                <form method="post" action="formSubmitGerirCompeticoes.php" id="tableForm">
                                    ...
                                    <table id="table" class="table table-hover" style="width:100% !important;">
                                        ...
                                        <tbody>
                                        <?php
                                        $queryGetEquipaByUser = "SELECT Escalao, Nome, epoca.NomeEpoca, competicao.IDCompeticao  FROM `$dataBaseName`.competicao JOIN `$dataBaseName`.epoca ON competicao.IDEpoca = epoca.IDEpoca";
                                        $queryResult = mysqli_query($GLOBALS['ligacao'], $queryGetEquipaByUser);

                                        if ($queryResult->num_rows > 0){
                                            while($row = mysqli_fetch_row($queryResult)){
                                                ?>
                                                <tr class="clickable-row" style="cursor: pointer;" id="<?php echo $row[3] ?>">
    ...
    <script src="js/dataTables/dataTable-gerirCompeticoes.js"></script>
    <script>
        function getEpocas (){
            let arrayEpocas = [<?php echo json_encode($epocas); ?>]
            return arrayEpocas;
        }
